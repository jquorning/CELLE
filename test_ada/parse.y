...
package body Parser is
   ...
   YYNOERRORRECOVERY : constant := 1;
   --  Disable all error recovery processing in the parser push-down
   --  automaton.

   procedure yytestcase (X : Boolean) renames testcase;
   --  Make yytestcase() the same as testcase()

   YYPARSEFREENEVERNULL : constant := 1;
   --  Indicate that sqlite3ParserFree() will never be called with a
   --  null pointer.

--#ifdef SQLITE_AMALGAMATION
--# define sqlite3Parser_ENGINEALWAYSONSTACK 1
--#endif
   --  In the amalgamation, the parse.c file generated by lemon and
   --  the tokenize.c file are concatenated.  In that case,
   --  sqlite3RunParser() has access to the the size of the yyParser
   --  object and so the parser engine can be allocated from stack.
   --  In that case, only the sqlite3ParserInit() and
   --  sqlite3ParserFinalize() routines are invoked and the
   --  sqlite3ParserAlloc() and sqlite3ParserFree() routines can be
   --  omitted.

   subtype YYMALLOCARGTYPE is U64;
   --  Alternative datatype for the argument to the malloc() routine
   --  passed into sqlite3ParserAlloc().  The default is size_t.
   ...
   procedure Disable_Lookaside (Pparse : Parse_Access) is
   begin
      PParse.DisableLookaside      := True;  --  ++;
      PParse.Db.lookaside.BDisable := True;  --  ++;
   end Disable_Lookaside;
...
%include {
   --  For a compound SELECT statement, make sure p->pPrior->pNext==p
   --  for all elements in the list.  And make sure list length does
   --  not exceed  SQLITE_LIMIT_COMPOUND_SELECT.
   procedure Parser_Double_Link_Select
     (pParse : in pParse_access;
     p       : in Select_access)
   is
   begin
      if P.pPrior /= null then
         declare
            Pnext    : Select_access := null;
            PLoop    : Select_Access;
            MxSelect : Integer;
            CNT      : Integer := 0;
         begin
            Ploop := P;
            loop
               exit when Ploop = null;
               PLoop.pNext                := pNext;
               PLoop.SelFlags.SF_Compound := True;
               PNext := PLoop;
               PLoop := PLoop.PPrior;
               Cnt   := CNT + 1;
            end loop;
            if
              not P.SelFlags.SF_MultiValue and
              (mxSelect = PParse.Db.ALimit      --  Assingment !!! XXX !!!
                 (SQLITE_LIMIT_COMPOUND_SELECT)) > 0 and
              Cnt > mxSelect
            then
               Sqlite3_Error_Msg (pParse, "too many terms in compound SELECT");
            end if;
         end;
            end if;
   end Parser_Double_Link_Select;
}
...
%include {
   ...
   --  new Expr to populate pOut.  Set the span of pOut to be the
   --  identifier that created the expression.
   function Token_Expr
     (Pparse : in Parse_Access;
      OP     : in Integer;
      T      : in Token)
      return Expr_Access
   is
      P : Expr_Access := Sqlite3DbMallocRawNN (PParse.db, sizeof(Expr)
                                                 + T.N + 1);
   begin
      if p /= null then
         ...
         P.u.ZToken (t.N) := 0;
         if Sqlite3_Is_quote (P.u.ZToken (0)) then
            Sqlite3_Dequote_Expr (p);
         end if;
#if SQLITE_MAX_EXPR_DEPTH>0
         P.nHeight := 1;
#endif
         if IN_RENAME_OBJECT then
            return (Expr*)Sqlite3_Rename_Token_Map (pParse, (void*)p, &t);
         end if;
      end if;
      return p;
   end Token_Expr;
...
%include {
   --  A routine to convert a binary TK_IS or TK_ISNOT expression into
   --  a  unary TK_ISNULL or TK_NOTNULL expression.
   procedure Binary_To_Unary_If_Null (Parse *pParse, Expr *pY, Expr
                                      *pA, int op)
   is
      DB : Sqlite3_access := PParse.DB;
   begin
      if
        pA /= null      and
        pY /= null      and
        PY.Op = TK_NULL and
        not IN_RENAME_OBJECT
      then
         PA.op := U8 (Op);
         Sqlite3ExprDelete (db, PA.pRight);
         PA.pRight = 0;
      end if;
   end Binary_To_Unary_If_Null;
}
...
%include {
   ...
   function Parser_Add_Expr_Id_List_Term
     (pParse     : PParse_Access;
      Pprior     : Expr_List_Access;
      PIdtoken   : Token_Access;
      HasCollate : Integer;
      SortOrder  : Integer)
      return Expr_List_Access
   is
      P : Expr_List_access := Sqlite3_Expr_List_Append (pParse, pPrior, 0);
   begin
      if
        (hasCollate or SortOrder /= SQLITE_SO_UNDEFINED)
        and PParse.Db.init.Busy = 0
      then
         Sqlite3_Error_Msg (pParse, "syntax error after column name \"%.*s\"",
                            PIdToken.n, PIdToken.z);
      end if;
      Sqlite3_Expr_List_Set_Name (pParse, p, pIdToken, 1);
      return p;
   end Parser_Add_Expr_Id_List_Term;
} // end %include
