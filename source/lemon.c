...
char *lemon_set_new(void);               /* A new set for element 0..N */
void lemon_set_free(char*);             /* Deallocate a set */
int lemon_set_add(char*,int);            /* Add element to a set */
int lemon_set_union(char *,char *);    /* A <- A U B, thru element N */
//#define SetFind(X,Y) (X[Y])       /* True if Y is in set X */
int lemon_set_find(char *,int);
    ...
    for(i=0; i<lemp->nstate; i++){
      for(cfp=lemp->sorted[i]->cfp; cfp; cfp=cfp->next){
        ...
        for(plp=cfp->fplp; plp; plp=plp->next){
          change = lemon_set_union(plp->cfp->fws,cfp->fws);
          if( change ){
