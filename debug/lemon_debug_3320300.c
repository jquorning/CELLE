...
{
  int i;
  int symbol_last = lem->nsymbol;

  if (mode == 1) symbol_last += 4; // Add ouput of '{default}, id, ids, number' too (sqlite.y)

  for (i=0; i < symbol_last; i++)
    {
      printf ("SYM %s INDEX %d NSUB %d KIND %d",
              lem->symbols[i]->name,
              lem->symbols[i]->index,
              lem->symbols[i]->nsubsym,
              lem->symbols[i]->type);

      if (mode == 1)
        {
          if (lem->symbols[i]->type == MULTITERMINAL)
            {
              int j;

              printf (" PREC (");
              for (j=0; j<lem->symbols[i]->nsubsym; j++)
                printf ("%d ", lem->symbols[i]->subsym[j]->prec);
              printf (")");
            }
          else
            printf (" PREC %d", lem->symbols[i]->prec);

          //  Put lambda
          printf (" LAMB %s", lem->symbols[i]->lambda ? "TRUE" : "FALSE");

          //  Put First_Set

          printf (" FS ");
          if (lem->symbols[i]->firstset == NULL)
            printf ("<null>");
          else
            {
...
/*                 printf ("%s", SetFind (lem->symbols [i]->firstset, bit) ? "1" : " "); */
            }
...
void debug_dump_rules (struct lemon *lem, int mode)
{
  struct rule *r = lem->rule;
  while (r)
    {
      printf ("RULE INDEX %d RULE %d LINE %d RULELINE %d",
              r->index,
              r->iRule,
              r->line,
              r->ruleline);
                     ...
                     struct config*  cfg)
{
  for (;cfg;cfg=cfg->next)
    {
      printf ("  RULE %d DOT %d STATUS %s FS ",
              cfg->rp->index,
              cfg->dot,
...
void debug_dump_states (struct lemon *lem, int mode)
{
  ...
  for (index = 0; index < lem->nstate; index++) 
    //  for (stat=*lem->sorted; stat; stat=stat->next)
    {
      //      debug_put_state (stat);
      debug_put_state (lem->sorted[index]);
      printf ("\n");
      if (mode == 1)
        {
          // debug_put_configs (stat->config);
          debug_put_configs (lem, lem->sorted[index]->cfp);
          //          printf ("\n");
        }
    }
}
